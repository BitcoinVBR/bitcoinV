...
UniValue CreateColdStakeDelegation(const UniValue& params, CWallet* const pwallet)
{
    ...
    if (pwallet->GetLastBlockHeight()) {
        throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Cold Staking is enabled \n"));
    }
    ...
    if (!IsValidDestination(stakeAddr))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinVPoS address");
    ...
    if (params.size() > 1 && !params[1].isNull() && !params[1].get_str().empty()) {
        ...
        if (!IsValidDestination(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinVPoS spending address.");
...
static RPCHelpMan delegatestake()
{
    return RPCHelpMan{"delegatestake",
                ...
                HELP_REQUIRING_PASSPHRASE,
                {
                    {"staker_address", RPCArg::Type::STR, RPCArg::Optional::NO, "The BitcoinVPoS address to delegate to."},
                    {"owner_address", RPCArg::Type::STR, RPCArg::Optional::NO, "The BitcoinVPoS address corresponding to the key that will be able to spend the stake."},
